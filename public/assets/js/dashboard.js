...
function renderEngineBarChart() {
  ...
  el.innerHTML = engineChartData.map((e, i) => {
    const w = Math.round((e.mal / maxMal) * 100);
    return '<div class="ebar-row" tabindex="0" aria-label="'+esc(e.name)+': '+e.mal+' detecções, '+e.rate.toFixed(0)+'% de taxa" '+
      'onmouseenter="showEbarTip(event,'+i+')" onmouseleave="hideEbarTip()" '+
      'onfocus="showEbarTip(event,'+i+')" onblur="hideEbarTip()">'+
      '<span class="ebar-label" title="'+esc(e.name)+'">'+esc(e.name)+'</span>'+
      '<div class="ebar-track"><div class="ebar-fill" style="width:'+w+'%"></div></div>'+
